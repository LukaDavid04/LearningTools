  ...
  return (
    <div className="grid lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="overflow-hidden">
            <div className="relative">
              ...
              <div className="absolute -bottom-10 left-6 h-24 w-24 sm:h-28 sm:w-28 rounded-3xl ring-2 ring-white/70 dark:ring-black/40 shadow-xl overflow-hidden flex items-center justify-center">
                <img src={avatar} alt="Luka" className="h-full w-full object-cover" />
              </div>
            </div>
        ...
        <section>
          <h2 className="text-2xl font-semibold tracking-tight mb-3">What I focus on</h2>
          <div className="grid sm:grid-cols-2 gap-4">
            {PROFILE.principles.map((b) => (
              <motion.div key={b.title} initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: .35 }}>
                <Card>
        ...
        <div className="rounded-2xl border bg-card/80 backdrop-blur shadow-sm">
          <div className="p-6 pb-2">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              I’m open to interesting problems, collaborative builds, and learning opportunities.
            </p>

            <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-2">
              <a href={`mailto:${PROFILE.email}`} className="flex items-center justify-center gap-2 rounded-xl border px-4 py-2 text-sm hover:bg-secondary/70 transition">
                Email
              </a>
              <a href={PROFILE.links[0].href} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 rounded-xl border px-4 py-2 text-sm hover:bg-secondary/70 transition">
                GitHub
              </a>
              <a href={PROFILE.links[1].href} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 rounded-xl border px-4 py-2 text-sm hover:bg-secondary/70 transition">
                LinkedIn
              </a>
            </div>
          </div>
